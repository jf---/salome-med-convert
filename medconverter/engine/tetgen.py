...
class MedConverterTetgen(MedConverterMesh):
    @staticmethod
    def convert_tetgen_to_med(filename_tetgen, filename_med, output_comm, verbose=False):
    ...
    @staticmethod
    def convert_med_to_tetgen(filename_med, filename_tetgen, output_comm, verbose=False):

        tic = time.perf_counter()
        c = MedConverterTetgen()
        c.verbose = verbose
        c.read_med_mesh(filename_med)
        c.create_tetgen_mesh()
        c.write_tetgen_mesh(filename_tetgen)
        toc = time.perf_counter()
        logger.debug("Mesh converted (in %0.4f seconds)" % (toc - tic))

    def __init__(self):
        super(MedConverterTetgen, self).__init__()
        self.tetgenmesh = None

    def read_tetgen_mesh(self, filename, parse_fast=True):
        ...
        with open(filename, "r", encoding=self._get_file_encoding(filename)) as file:
            ...
            for i_tet in range(nb_tetra):
                ...
                nodes_med = c_renum.external_to_medcoupling(tet_medcoupling_type, nodes_tet)
            ...
            toc = time.perf_counter()
            logger.debug(" Load %d tetrahedra (in %0.4f seconds)" % (nb_tetra, toc - tic))
    ...
    def write_tetgen_mesh(self, filename):
        raise NotImplementedError()

    def create_tetgen_mesh(self):
        raise NotImplementedError()
