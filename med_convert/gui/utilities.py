...
def set_mandatory(label, mandatory=True):
    """
    Decorate label as a mandatory one.

    Arguments:
        label (QLabel): Label being decorated.
        mandatory (Optional[bool]): *True* to make label mandatory;
            *False* otherwise. Defaults to *True*.
    """
    palette = label.palette()
    color = Q.Qt.red if mandatory else \
        Q.QApplication.palette().color(Q.QPalette.Foreground)
    palette.setColor(label.foregroundRole(), color)
    label.setPalette(palette)
    text = label.text()
    if mandatory and not text.endswith(mandatory_suffix()):
        text = text + mandatory_suffix()
    elif not mandatory and text.endswith(mandatory_suffix()):
        text = text[:-len(mandatory_suffix())]
    label.setText(text)


def update_palette(widget):
    """
    Update widget's palette in accordance with its input validity state.

    Arguments:
        widget (QWidget): Input widget.
    """
    if hasattr(widget, 'hasAcceptableInput'):
        is_valid = widget.hasAcceptableInput()
        palette = widget.palette()
        color = palette.color(Q.QPalette.Foreground) if is_valid else Q.Qt.red
        palette.setColor(Q.QPalette.Text, color)
